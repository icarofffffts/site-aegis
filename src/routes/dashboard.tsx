...
import { 
  BarChart3, Shield, Users, Settings, Clock,
  CheckCircle2, XCircle, LayoutDashboard, AlertTriangle,
  LogOut, Wifi, WifiOff, Plus, Trash2, ToggleLeft, ToggleRight,
  Bell, Search,
} from "lucide-react";
            ...
            <div className="relative">
              ...
              >
                {selectedGuild ? (
                  <>
                    {selectedGuild.icon
                      ? <img src={selectedGuild.icon} alt={selectedGuild.name} className="h-6 w-6 rounded-full shrink-0" />
                      : <div className="h-6 w-6 rounded-full bg-blue-500 flex items-center justify-center text-[10px] font-bold text-white shrink-0">{selectedGuild.name[0]}</div>
                    }
                    <span className="text-sm font-semibold text-white truncate flex-1 text-left">{selectedGuild.name}</span>
                  </>
                ) : (
                  <>
                    {user?.avatar
                      ? <img src={user.avatar} alt={user.username} className="h-6 w-6 rounded-full shrink-0" />
                      : <div className="h-6 w-6 rounded-full bg-blue-500 flex items-center justify-center text-[10px] font-bold text-white shrink-0">{user?.username?.[0]?.toUpperCase() ?? "?"}</div>
                    }
                    <span className="text-sm font-semibold text-white truncate flex-1 text-left">{user?.username ?? "..."}</span>
                  </>
                )}
                <span className="text-[#7d8590] text-xs shrink-0">{showGuildPicker ? "▲" : "▼"}</span>
              </button>
              ...
              {showGuildPicker && (
                <div className="absolute top-full left-0 right-0 mt-1 rounded-lg border border-[#30363d] bg-[#161b22] shadow-2xl z-50 max-h-72 overflow-y-auto">
                  <div className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-[#7d8590] border-b border-[#30363d]">
                    {guilds === null ? "Carregando..." : `${guilds.mutual.length} servidor${guilds.mutual.length !== 1 ? "es" : ""} com Aegis`}
                  </div>
                  ...
                  {guilds?.mutual.map(g => (
                    <button key={g.id} onClick={() => { setSelectedGuild(g); setShowGuildPicker(false); setActiveTab("settings"); }}
                      ...
                      {g.icon
                        ...
                        : <div className="h-7 w-7 rounded-full bg-[#30363d] flex items-center justify-center text-xs font-bold text-white shrink-0">{g.name[0]}</div>
                      }
                      <div className="text-left min-w-0 flex-1">
                        <p className="text-sm font-medium text-white truncate">{g.name}</p>
                        {g.owner && <p className="text-[10px] text-[#7d8590]">Dono</p>}
                      </div>
                      {selectedGuild?.id === g.id && <CheckCircle2 className="h-4 w-4 text-green-500 ml-auto shrink-0" />}
                    </button>
                  ))}
                  {(guilds?.notAdded?.length ?? 0) > 0 && (
                    <>
                      ...
                      {guilds?.notAdded.map(g => (
                        <a key={g.id} href={`https://discord.com/oauth2/authorize?client_id=1485085619280679145&permissions=8&scope=bot%20applications.commands&guild_id=${g.id}`}
                          ...
                          {g.icon
                            ? <img src={g.icon} alt={g.name} className="h-7 w-7 rounded-full shrink-0 grayscale" />
                            : <div className="h-7 w-7 rounded-full bg-[#30363d] flex items-center justify-center text-xs font-bold text-[#7d8590] shrink-0">{g.name[0]}</div>
                          }
                          <div className="text-left min-w-0 flex-1">
...
function OverviewSection({ data, loading }: { data: DashboardData | null; loading: boolean }) {
  ...
  const STATS = [
    { label: "Usuários Flagados", value: s?.flaggedUsers ?? "—", icon: Users, color: "text-blue-500" },
    { label: "Alertas Hoje", value: s?.alertsToday ?? "—", icon: Shield, color: "text-green-500" },
    { label: "Alertas Críticos", value: s?.criticalAlerts ?? "—", icon: AlertTriangle, color: "text-orange-500" },
    { label: "Total de Alertas", value: s?.totalAlerts ?? "—", icon: Clock, color: "text-purple-500" },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {STATS.map((stat) => (
          <div key={stat.label} className="p-6 rounded-xl border border-[#30363d] bg-[#161b22]/50">
            ...
            <div className="text-2xl font-bold text-white">
              {loading ? <span className="inline-block h-7 w-16 rounded bg-[#30363d] animate-pulse" /> : stat.value}
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-xl border border-[#30363d] bg-[#161b22]/50 overflow-hidden">
          ...
          <div className="divide-y divide-[#30363d]">
            {loading ? (
              ...
              <div className="px-6 py-8 text-center text-sm text-[#7d8590]">Nenhuma ação registrada ainda.</div>
            ) : (
              data?.recentActions.map((item) => (
                <div key={item.id} className="px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`h-8 w-8 rounded-full flex items-center justify-center ${actionColor[item.action_type] ?? "bg-[#30363d] text-[#7d8590]"}`}>
                      {item.action_type === "banned" && <XCircle className="h-4 w-4" />}
                      {item.action_type === "warned" && <AlertTriangle className="h-4 w-4" />}
                      {(item.action_type === "kicked" || item.action_type === "muted") && <Shield className="h-4 w-4" />}
        ...
        <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 p-6 flex flex-col items-center justify-center text-center">
          <div className={`h-20 w-20 rounded-full flex items-center justify-center mb-4 border-4 ${s?.botOnline ? "border-green-500 bg-green-500/10" : "border-red-500 bg-red-500/10"}`}>
            {s?.botOnline
          ...
          <p className={`text-sm mt-1 font-medium ${s?.botOnline ? "text-green-500" : "text-red-400"}`}>
...
function AlertsSection({ authFetch }: { authFetch: typeof window.fetch }) {
    ...
    authFetch("/api/botconfig")
      ...
      .then(d => {
        if (d?.flaggedUsers) {
          ...
          setAlerts(list.sort((a, b) => new Date(b.lastSeen).getTime() - new Date(a.lastSeen).getTime()));
        }
      })
      .catch(() => {})
  ...
  const filtered = alerts.filter(a => {
    const matchSearch = !search || a.userId.includes(search) || a.types.join(",").includes(search.toLowerCase());
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#7d8590] mt-1">Usuários com alertas registrados no sistema AegisBot.</p>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <span className="text-[#7d8590]">{alerts.length} total</span>
          <span className="text-red-400 font-medium">{alerts.filter(a => a.maxSeverity === "critical").length} críticos</span>
        </div>
      </header>
      ...
      <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 overflow-hidden">
        ...
        <div className="divide-y divide-[#30363d]">
          {loading ? (
            ...
            filtered.map(alert => (
              <div key={alert.userId} className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-6 py-4 items-center hover:bg-[#161b22] transition-colors">
                ...
                <div className="flex flex-wrap gap-1 justify-end">
                  {alert.types.slice(0, 3).map((t: string) => (
                    <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-[#30363d] text-[#7d8590]">{t}</span>
                  ))}
                </div>
                <span className="text-sm font-bold text-white text-center">{alert.count}</span>
                <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${SEV_COLOR[alert.maxSeverity]}`}>
                  {alert.maxSeverity}
                </span>
              </div>
            ))
...
function ProtectionSection({ authFetch, guildId }: { authFetch: typeof window.fetch; guildId?: string }) {
  ...
  const addPattern = async () => {
    ...
    await authFetch("/api/protection", {
      ...
      body: JSON.stringify({ guildId: GUILD_ID, patternType: newPattern.type, patternValue: newPattern.value, severity: newPattern.severity, description: newPattern.description }),
    });
  ...
  const SEV_COLOR: Record<string, string> = { critical: "text-red-400", high: "text-orange-400", medium: "text-yellow-400", low: "text-blue-400" };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 p-6 space-y-4">
        ...
        {[
          { key: "disclosure_detection_enabled", label: "Detecção silenciosa ativa", desc: "Analisa mensagens em background sem resposta pública" },
          { key: "auto_kick_on_critical", label: "Ação automática em críticos", desc: "Reage e loga automaticamente quando severidade crítica é detectada" },
        ].map(({ key, label, desc }) => (
          <div key={key} className="flex items-center justify-between py-3 border-b border-[#30363d] last:border-0">
            <div>
              <p className="text-sm font-medium text-white">{label}</p>
              <p className="text-xs text-[#7d8590] mt-0.5">{desc}</p>
            </div>
            <button onClick={() => toggleConfig(key, !config?.[key])} className="text-[#7d8590] hover:text-white transition-colors">
              {config?.[key]
      ...
      <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 overflow-hidden">
        <div className="px-6 py-4 border-b border-[#30363d] flex items-center justify-between">
          ...
          <span className="text-xs text-[#7d8590]">{patterns.length} padrão{patterns.length !== 1 ? "s" : ""}</span>
        </div>

        {/* Adicionar novo */}
        <div className="px-6 py-4 border-b border-[#30363d] bg-[#0d1117] flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[160px]">
            <label className="text-[10px] text-[#7d8590] uppercase font-bold mb-1 block">Valor</label>
            <input value={newPattern.value} onChange={e => setNewPattern(p => ({ ...p, value: e.target.value }))}
              placeholder="ex: betano, /cassino/i" className="w-full px-3 py-1.5 rounded-md bg-[#161b22] border border-[#30363d] text-sm text-white focus:outline-none focus:border-blue-500" />
          </div>
          <div>
            <label className="text-[10px] text-[#7d8590] uppercase font-bold mb-1 block">Tipo</label>
            <select value={newPattern.type} onChange={e => setNewPattern(p => ({ ...p, type: e.target.value }))}
          ...
          <div>
            <label className="text-[10px] text-[#7d8590] uppercase font-bold mb-1 block">Severidade</label>
          ...
          <Button onClick={addPattern} disabled={saving || !newPattern.value.trim()} size="sm" className="bg-blue-600 hover:bg-blue-700 text-white border-none h-9">
        ...
        <div className="divide-y divide-[#30363d]">
          {loading ? (
            ...
            <div className="px-6 py-8 text-center text-sm text-[#7d8590]">Nenhum padrão customizado. Os padrões globais do AegisBot continuam ativos.</div>
          ) : (
            patterns.map(p => (
              <div key={p.id} className="px-6 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-[#30363d] ${SEV_COLOR[p.severity]}`}>{p.severity}</span>
                  <span className="text-xs text-[#7d8590] bg-[#0d1117] px-2 py-0.5 rounded">{p.pattern_type}</span>
                  <span className="text-sm text-white font-mono">{p.pattern_value}</span>
                </div>
                <button onClick={() => removePattern(p.id)} className="text-[#7d8590] hover:text-red-400 transition-colors">
...
function StatsSection({ authFetch }: { authFetch: typeof window.fetch }) {
  ...
  const maxAlerts = days.length ? Math.max(...days.map(d => stats.alertsByDay[d]?.total ?? 0), 1) : 1;

  const SEV_COLOR: Record<string, string> = { critical: "bg-red-500", high: "bg-orange-500", medium: "bg-yellow-500", low: "bg-blue-500" };
  const TYPE_LABEL: Record<string, string> = { disclosure: "Divulgação", spam: "Spam", scam: "Golpe", custom: "Customizado" };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {loading ? (
          <div className="h-32 flex items-end gap-2">
            {Array.from({ length: 14 }).map((_, i) => (
              <div key={i} className="flex-1 rounded-t bg-[#30363d] animate-pulse" style={{ height: `${20 + Math.random() * 60}%` }} />
            ))}
          </div>
        ) : days.length === 0 ? (
          <p className="text-sm text-[#7d8590] text-center py-8">Nenhum dado nos últimos 14 dias.</p>
        ) : (
              ...
              return (
                <div key={day} className="flex-1 flex flex-col items-center gap-1 group">
                  <div className="relative w-full">
                    ...
                    <div className="w-full rounded-t bg-blue-500/70 hover:bg-blue-500 transition-colors" style={{ height: `${height * 1.28}px` }} />
                  </div>
                  <span className="text-[9px] text-[#7d8590] rotate-45 origin-left mt-1">{day.slice(5)}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 p-6">
          ...
          {loading ? <div className="space-y-3">{Array.from({length:4}).map((_,i)=><div key={i} className="h-6 rounded bg-[#30363d] animate-pulse"/>)}</div> : (
            <div className="space-y-3">
              {["critical","high","medium","low"].map(sev => {
                const count = stats?.bySeverity?.[sev] ?? 0;
                const total = Object.values(stats?.bySeverity ?? {}).reduce((a: number, b: any) => a + b, 0) as number;
                const pct = total ? Math.round((count / total) * 100) : 0;
                return (
                  <div key={sev}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-white capitalize">{sev}</span>
                      <span className="text-[#7d8590]">{count} ({pct}%)</span>
                    </div>
                    <div className="h-2 rounded-full bg-[#30363d]">
                      <div className={`h-2 rounded-full ${SEV_COLOR[sev]}`} style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                );
              })}
          ...
          {loading ? <div className="space-y-3">{Array.from({length:4}).map((_,i)=><div key={i} className="h-6 rounded bg-[#30363d] animate-pulse"/>)}</div> : (
            <div className="space-y-3">
              {Object.entries(stats?.byType ?? {}).sort(([,a],[,b]) => (b as number)-(a as number)).map(([type, count]) => (
                <div key={type} className="flex items-center justify-between py-2 border-b border-[#30363d] last:border-0">
      ...
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          {guild.icon
            ...
            : <div className="h-12 w-12 rounded-xl bg-[#30363d] flex items-center justify-center text-lg font-bold text-white">{guild.name[0]}</div>
          }
          <div>
        ...
        <Button onClick={save} disabled={saving || loading} className="bg-[#1f883d] hover:bg-[#1a7a35] text-white border-none">
      ...
      {loading ? (
        <div className="space-y-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-16 rounded-xl bg-[#161b22] border border-[#30363d] animate-pulse" />
          ))}
        </div>
      ) : (
        <>
          <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 px-6">
            <h3 className="font-semibold text-white pt-5 pb-2 text-sm uppercase tracking-wider text-[#7d8590]">Canais</h3>
            <Field label="Canal de logs" desc="ID do canal onde o Aegis envia logs de moderação e alertas.">
              <input value={form.alert_log_channel_id} onChange={e => setForm(f => ({ ...f, alert_log_channel_id: e.target.value }))}
                placeholder="ID do canal" className="w-52 px-3 py-1.5 rounded-md bg-[#0d1117] border border-[#30363d] text-sm text-white font-mono focus:outline-none focus:border-blue-500" />
            </Field>
            <Field label="Canal de boas-vindas" desc="ID do canal onde o Aegis posta mensagens de boas-vindas públicas.">
              <input value={form.welcome_channel_id} onChange={e => setForm(f => ({ ...f, welcome_channel_id: e.target.value }))}
                placeholder="ID do canal" className="w-52 px-3 py-1.5 rounded-md bg-[#0d1117] border border-[#30363d] text-sm text-white font-mono focus:outline-none focus:border-blue-500" />
            </Field>
          </div>

          <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 px-6">
            <h3 className="font-semibold text-white pt-5 pb-2 text-sm uppercase tracking-wider text-[#7d8590]">Boas-vindas</h3>
            <Field label="Mensagem de boas-vindas" desc="Enviada por DM ao novo membro. Use {user} para o nome.">
              <textarea value={form.welcome_message} onChange={e => setForm(f => ({ ...f, welcome_message: e.target.value }))}
                placeholder="Bem-vindo ao servidor, {user}! ..." rows={3}
          ...
          <div className="rounded-xl border border-[#30363d] bg-[#161b22]/50 px-6">
            <h3 className="font-semibold text-white pt-5 pb-2 text-sm uppercase tracking-wider text-[#7d8590]">Proteção</h3>
            <Field label="Detecção silenciosa" desc="Analisa mensagens em background e registra alertas sem resposta pública.">
              <button onClick={() => setForm(f => ({ ...f, disclosure_detection_enabled: !f.disclosure_detection_enabled }))} className="text-[#7d8590] hover:text-white transition-colors">
                {form.disclosure_detection_enabled ? <ToggleRight className="h-7 w-7 text-green-500" /> : <ToggleLeft className="h-7 w-7" />}
              </button>
            </Field>
            <Field label="Ação automática em críticos" desc="Reage e loga automaticamente quando uma ameaça crítica é detectada.">
              <button onClick={() => setForm(f => ({ ...f, auto_kick_on_critical: !f.auto_kick_on_critical }))} className="text-[#7d8590] hover:text-white transition-colors">
                {form.auto_kick_on_critical ? <ToggleRight className="h-7 w-7 text-green-500" /> : <ToggleLeft className="h-7 w-7" />}
              </button>
            </Field>
            <Field label="Threshold de raid" desc="Número de entradas simultâneas para ativar o anti-raid automático.">
              <input type="number" min="2" max="50" value={form.raid_threshold} onChange={e => setForm(f => ({ ...f, raid_threshold: e.target.value }))}
            ...
            <Field label="Idade mínima da conta (dias)" desc="Contas mais novas que este valor são monitoradas ao enviar links.">
              <input type="number" min="0" max="365" value={form.account_min_age_days} onChange={e => setForm(f => ({ ...f, account_min_age_days: e.target.value }))}
                className="w-20 px-3 py-1.5 rounded-md bg-[#0d1117] border border-[#30363d] text-sm text-white text-center focus:outline-none focus:border-blue-500" />
            </Field>
          </div>
