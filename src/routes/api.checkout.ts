        ...
        try {
          ...
          if (method === "stripe") {
            ...
            if (!STRIPE_SECRET_KEY) {
              ...
              return new Response(JSON.stringify({ url: "#", error: "Stripe não configurado" }), { status: 200 });
            }
            const stripe = require("stripe")(STRIPE_SECRET_KEY);
            const session = await stripe.checkout.sessions.create({
              mode: "subscription",
              payment_method_types: ["card"],
              line_items: [{ price_data: { currency: "brl", product_data: { name: `Aegis ${plan} - ${interval === "year" ? "Anual" : "Mensal"}` }, unit_amount: amount }, quantity: 1 }],
          ...
          if (method === "mercadopago") {
            ...
            if (!MP_ACCESS_TOKEN) {
              return new Response(JSON.stringify({ url: "#", error: "Mercado Pago não configurado" }), { status: 200 });
            }
            ...
            const resp = await fetch("https://api.mercadopago.com/checkout/preferences", {
              ...
              body: JSON.stringify({
                items: [{ title: `Aegis ${plan} - ${interval === "year" ? "Anual" : "Mensal"}`, quantity: 1, currency_id: "BRL", unit_price: amount / 100 }],
                back_urls: { success: `${process.env.NEXT_PUBLIC_SITE_URL}/dashboard?checkout=success`, failure: `${process.env.NEXT_PUBLIC_SITE_URL}/checkout?plan=${plan}` },
            ...
            return new Response(JSON.stringify({ url: "#", error: "Erro ao criar preferência MP" }), { status: 200 });
          }
